#include<iostream>
#include "asciiArt.hpp"

using std::cout;
using std::endl;

//Art by From Ric_Hotchkiss_sdrc_com
//http://www.ascii-code.com/ascii-art/buildings-and-places/houses.php
void asciiShelter()
{
	cout << endl;


	cout << "                    	"<<endl;
	cout << "                       	"<<endl;
	cout << "                     	"<<endl;
	cout << "                    	"<<endl;
	cout << "         /\\        "<<endl;
	cout << "        / ^\\   /\\ "<<endl;
	cout << "       /^   \\ / ^\\  "<<endl;
	cout << "      /  ^ ^ \\^  ^\\                   ____               /|\\	"<<endl;
	cout << "     / ^ ^  ^ \\^  _\\__________________|  |_____         /||o\\	"<<endl;
	cout << "    / ^^  ^ ^ ^\\ /______________________________\\      /|o|||\\	"<<endl;
	cout << "   /  ^  ^^ ^ ^  /________________________________\\   /|||||o|\\	"<<endl;
	cout << "  /^ ^  ^ ^^  ^    ||___|___||||||||||||___|__|||    /||o||||||\\	"<<endl;
	cout << " / ^   ^   ^    ^  ||___|___||||||||||||___|__|||        | |	"<<endl;
	cout << "/ ^ ^ ^  ^  ^  ^   ||||||||||||||||||||||||||||||oooooooo| |ooooooo"<<endl;
	cout << "ooooooooooooooooooooooooooooooooooooooooooooooooooooooooo"<<endl;

	cout << endl;
}


//Art by From Ric_Hotchkiss_sdrc_com
//http://www.ascii-code.com/ascii-art/buildings-and-places/houses.php
void asciiVillNoShelter()
{
	cout << endl;


	cout << "                    	" << endl;
	cout << "                       	" << endl;
	cout << "                     	" << endl;
	cout << "                    	" << endl;
	cout << "         /\\        " << endl;
	cout << "        / ^\\   /\\ " << endl;
	cout << "       /^   \\ / ^\\  " << endl;
	cout << "      /  ^ ^ \\^  ^\\                                      /|\\	" << endl;
	cout << "     / ^ ^  ^ \\^   \\                                    /||o\\	" << endl;
	cout << "    / ^^  ^ ^ ^\\    \\                                  /|o|||\\	" << endl;
	cout << "   /  ^  ^^ ^ ^                                       /|||||o|\\	" << endl;
	cout << "  /^ ^  ^ ^^  ^                                      /||o||||||\\	" << endl;
	cout << " / ^   ^   ^    ^                                        | |	" << endl;
	cout << "/ ^ ^ ^  ^  ^  ^   oooooooooooooooooooooooooooooooooooooo| |ooo" << endl;
	cout << "ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo" << endl;

	cout << endl;
	cout << endl; 
}


//courtesy of ascii text generator at http://patorjk.com/software/taag/
void asciiYouLose()
{
	cout << endl;

	cout<< "##    ##  #######  ##     ##    ##        #######   ######  ######## "<<endl;
	cout<< " ##  ##  ##     ## ##     ##    ##       ##     ## ##    ## ##       "<<endl;
	cout<< "  ####   ##     ## ##     ##    ##       ##     ## ##       ##       "<<endl;
	cout<< "   ##    ##     ## ##     ##    ##       ##     ##  ######  ######   "<<endl;
	cout<< "   ##    ##     ## ##     ##    ##       ##     ##       ## ##       "<<endl;
	cout<< "   ##    ##     ## ##     ##    ##       ##     ## ##    ## ##       "<<endl;
	cout<< "   ##     #######   #######     ########  #######   ######  ######## "<<endl;

	cout << endl;
	cout << endl;
}